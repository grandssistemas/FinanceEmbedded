...
import modalTemplate from '../views/receiveTitlePrintModal.html';
...
function TitleListEmbeddedController(
	...
	$scope.advancedSearch = (param) => {
		const filterType = new GQuery(new Criteria('obj.titleType', ComparisonOperator.EQUAL, $scope.titleType.toUpperCase()));
		if (!param || !param.and) {
			param = filterType;
		} else {
...
export default TitleListEmbeddedController;
