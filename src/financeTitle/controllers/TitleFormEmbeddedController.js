import template from './../views/modalLaunchPaid.html';
import templatePayments from './../views/PaymentsModal.html';
...
function TitleFormEmbeddedController(
	...
	function createMethods() {
		...
		$scope.ratioPlanSearch = (param) => {
			param = param || '';
			const search = $scope.ratioPlan.methods.asyncSearchWithGQuery(new GQuery(new Criteria('obj.name', ComparisonOperator.CONTAINS, param).addTranslate().addIgnoreCase()));
			return search;
		};
		...
		$scope.selectedRatio = function (item, planType, index) {
			if (item.isTag) {
				const position = $scope.title.data.planLeafs[index].length - 1;
				const leaf = {
					name: item.name,
					planType
				};
		...
		$scope.automaticRatio = function (value) {
			...
			const total = $scope.sumParcels($scope.title.data.parcel);
		...
		$scope.buscaLeafs = function (plan, query) {
			...
			const search = `obj.planType.id=${plan.type.id} and obj.name like '%${query}%'`;
		...
		if ($scope.$ctrl.operation && $scope.$ctrl.operation === 'REPLECEMENT') {
			...
			angular.forEach($scope.parcels, (parcel, index) => {
				...
				if ($scope.parcels.length === (index + 1)) {
					TitleService.getParcelsDTO($scope.idParcels).then((data) => {
		...
		$scope.getDataFromBarcode = function () {
			const barcode = $scope.title.data.barcode;
			if (barcode) {
		...
		$scope.addLeaf = function (account) {
			let notFind = 0;
			const leaf = angular.copy(account.leaf);
		...
		$scope.change = function () {
			if (!$scope.disable) {
				...
				$timeout(() => {
					...
					if (objScroll) { objScroll.scrollTop = objScroll.scrollHeight; }
				}, 700);
			}
		};
		...
		// Calcula parcelas
		const setExpiration = function (parcel, expiration) {
		...
		$scope.calculateParcels = function () {
			const valueParcel = $scope.title.data.value;
			const numberParcel = $scope.title.data.numberParcel;
			const valueParcelReceive = parseFloat((valueParcel / numberParcel).toFixed(2));
			const valueRemainingCalculateReceive = parseFloat((valueParcel - (valueParcelReceive * numberParcel)).toFixed(2));
			...
			const expiration = new Date($scope.title.data.expiration);
		...
		$scope.sumParcels = function (array) {
			return array.reduce((p, n) => p += n.value, 0);
		};
				...
				SweetAlert.swal(
					...
					(isConfirm) => {
						if (isConfirm) {
							...
							let restValue = $scope.totalSumParcel - totalParcels;
							let countParcels = $scope.title.data.parcel.length - index;
							let parcelValue = restValue / countParcels;
							let total = 0;
							let count = 0;
